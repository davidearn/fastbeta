...
#'   }
#'
#'   `B` is optional if `hatN0` and `nu` are defined in `par_list`
#'   (see Details).
...
#'   }
#'
#'   `hatN0` and `nu` are optional if `B` is defined in `df`
#'   (see Details).
#'
#' @return
...
#'   }
#'   \item{`Z_agg`}{Aggregated incidence. `Z_agg[i]` is the estimated
#'     number of infections between times `t[i-round(tgen)+1]` and
#'     `t[i]`, for `i` in `seq(1 + round(tgen), nrow(df), by = round(tgen))`.
