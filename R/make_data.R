...
#'
#' Simulations account for up to three sources of randomness:
#'
...
#'      event probabilities are determined by terms in the ODE. Method:
#'      [adaptivetau::ssa.adaptivetau()].
#'    * Otherwise, observations are generated by numerically
#'      integrating the ODE. Method: [deSolve::lsoda()].
#'    * Made reproducible by specifying `seeds[2]`.
#'
#' 3. Random under-reporting of cases
#'
...
#' }
#'
#' It possesses `par_list`, `n`, `with_dem_stoch`, and `seeds` as
#' attributes.
#'
#' @md
...
attr(df, "n")              <- n
attr(df, "with_dem_stoch") <- with_dem_stoch
attr(df, "seeds")          <- seeds
df
}
